...
function writeSavedReports(reports) {
  try {
    const localOnly = (reports || []).filter((report) => !report.shared);
    window.localStorage.setItem(REPORTS_STORAGE_KEY, JSON.stringify(localOnly));
  } catch (error) {}
}

function normalizeSharedReport(report) {
  return {
    id: report.id || `shared-${Date.now()}`,
    schoolId: report.schoolId || report.school_id || "",
    schoolName: report.schoolName || report.school_name || "",
    officeCode: report.officeCode || report.office_code || "",
    officeName: report.officeName || report.office_name || "",
    type: report.type || "위험 제보",
    memo: report.memo || "",
    lat: Number(report.lat || 0),
    lon: Number(report.lon || 0),
    locationLabel: report.locationLabel || report.location_label || "공유 제보 위치",
...
function mergeReports(localReports, sharedReports) {
  const map = new Map();
  [...(sharedReports || []).map(normalizeSharedReport), ...(localReports || [])].forEach((report) => {
    if (!report?.id) return;
...
function buildSharedReportPayload(report, school, office) {
  return {
    schoolId: school?.id || "",
    schoolName: school?.name || "",
    officeCode: office?.code || school?.officeCode || "",
    officeName: office?.name || school?.officeName || "",
    schoolLat: Number(school?.lat || 0),
    schoolLon: Number(school?.lon || 0),
    lat: Number(report?.lat || 0),
    lon: Number(report?.lon || 0),
    type: report?.type || "위험 제보",
    severity: report?.severity || "보통",
    memo: report?.memo || "",
...
function useAdminAuth() {
  ...
  const [adminPassword, setAdminPassword] = useState(() => sessionStorage.getItem("gilnuri-dev-password") || "");
  ...
  async function login(password) {
    ...
    try {
      ...
      sessionStorage.setItem("gilnuri-dev-password", password);
  ...
  function logout() {
    ...
    sessionStorage.removeItem("gilnuri-dev-password");
    setAdminPassword("");
    setIsAdmin(false);
  }
  return { isAdmin, adminPassword, adminError, login, logout };
}
...
function riskBarColor(value, toneLabel) {
  if (toneLabel === "안전") return "#34d399";
  if (toneLabel === "주의") return "#f59e0b";
  if (toneLabel === "위험") return "#fb7185";
...
function Bar({ label, value, danger = false, toneLabel }) {
  const width = clamp(value, 0, 100);
  const barColor = danger ? riskBarColor(width, toneLabel) : "#34d399";
  return <div style={{ marginTop: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, fontSize: 14, fontWeight: 900 }}><span>{label}</span><span>{Math.round(width)}</span></div><div style={{ marginTop: 8, height: 11, background: "#f1f5f9", borderRadius: 999, overflow: "hidden" }}><div style={{ height: "100%", width: `${width}%`, background: barColor, borderRadius: 999 }} /></div></div>;
}
    ...
    async function renderLeafletMap() {
      ...
      try {
        ...
        (userReports || []).filter((report) => report.lat && report.lon).slice(0, 30).forEach((report) => {
          ...
          const marker = L.marker(latLng, { icon }).bindPopup(`<b>${escapeHtml(report.type || "위험 제보")}</b><br/>${escapeHtml(report.memo || "상세 설명 없음")}`);
          marker.addTo(map);
          layersRef.current.push(marker);
        });
...
function UsageGuideCard({ setTab }) {
  const steps = [
    {
      number: "1",
      title: "기본 설정에서 도착지 학교 선택",
      desc: "왼쪽 기본 설정에서 시도교육청, 학교 검색, 학교급, 도착지 학교를 고릅니다. 길누리는 선택한 학교를 최종 도착지로 보고 주변 보호구역과 안전 인프라를 분석합니다.",
      action: "도착지 학교 선택",
      target: null,
      tone: "blue",
    },
    {
      number: "2",
      title: "설정 탭에서 출발지 설정",
      desc: "집 주소, 현재 위치, 직접 좌표 입력 중 하나로 출발지를 설정합니다. 출발지가 있어야 실제 거리, 도보 시간, 경로 비교, 실제 통학 지도가 제대로 계산됩니다.",
      action: "설정 탭으로 이동",
      target: "settings",
      tone: "green",
    },
    {
      number: "3",
      title: "위험 제보 탭에서 위험 위치 등록",
      desc: "지도에서 위험한 위치를 누른 뒤 불법 주정차, 신호등 고장, 공사·장애물, 보도 파손 같은 위험 유형을 제보할 수 있습니다. 등록된 제보는 점수와 실제 통학 지도에 반영됩니다.",
      action: "위험 제보하기",
      target: "report",
      tone: "red",
    },
    {
      number: "4",
      title: "실제 통학 지도에서 경로 확인",
      desc: "오늘의 등굣길 탭의 실제 통학 지도에서 학교, 출발지, 선택 경로, 보호구역, 위험 제보를 실제 좌표 기반으로 확인할 수 있습니다.",
      action: "오늘의 등굣길 보기",
      target: "today",
      tone: "dark",
    },
  ...
  const colors = {
    ...
    green: { bg: "#ecfdf5", border: "#bbf7d0", fg: "#15803d" },
    red: { bg: "#fff1f2", border: "#fecdd3", fg: "#be123c" },
    dark: { bg: "#f8fafc", border: "#cbd5e1", fg: "#0f172a" },
  };

  return <Card><div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "start", flexWrap: "wrap" }}><div><h2 style={{ margin: 0 }}>활용 가이드</h2><p style={{ margin: "8px 0 0", color: "#64748b", lineHeight: 1.7 }}>처음 사용할 때는 도착지 학교와 출발지를 먼저 설정한 뒤, 실제 통학 지도와 위험 제보 기능을 함께 확인하세요.</p></div><Badge tone="blue">처음 사용자용</Badge></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12, marginTop: 18 }}>{steps.map((step) => { const color = colors[step.tone] || colors.blue; return <div key={step.number} style={{ background: color.bg, border: `1px solid ${color.border}`, borderRadius: 20, padding: 18 }}><div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 34, height: 34, borderRadius: 999, background: color.fg, color: "white", display: "grid", placeItems: "center", fontWeight: 950 }}>{step.number}</div><b style={{ color: color.fg }}>{step.title}</b></div><p style={{ margin: "12px 0 0", color: "#334155", lineHeight: 1.7, fontSize: 14 }}>{step.desc}</p>{step.target ? <button onClick={() => setTab(step.target)} style={{ ...styles.button, marginTop: 12, background: color.fg, color: "white" }}>{step.action}</button> : <p style={{ margin: "12px 0 0", color: color.fg, fontSize: 13, fontWeight: 900 }}>{step.action}</p>}</div>; })}</div><div style={{ marginTop: 16, background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 18, padding: 16, color: "#92400e", lineHeight: 1.7, fontWeight: 850 }}>요약: 기본 설정에서 <b>도착지 학교</b>를 고르고, 설정 탭에서 <b>출발지</b>를 등록한 뒤, 오늘 안전 분석을 실행하면 실제 통학 지도에서 경로를 확인할 수 있습니다. 위험 지점은 위험 제보 탭에서 직접 등록할 수 있습니다.</div></Card>;
}
...
function ReportLocationPicker({ school, origin, analysisRadius, selectedPin, setSelectedPin }) {
  ...
  const mapRef = useRef(null);
  const layersRef = useRef([]);
  const [leafletStatus, setLeafletStatus] = useState("제보 지도를 준비 중입니다...");
  ...
  useEffect(() => {
    ...
    async function renderReportMap() {
      if (!mapEl.current || !schoolLat || !schoolLon) return;
      setLeafletStatus("제보 지도 불러오는 중...");
      ...
      try {
        ...
        const schoolLatLng = [schoolLat, schoolLon];
        const bounds = L.latLngBounds([schoolLatLng]);
        ...
        const schoolMarker = L.marker(schoolLatLng, { icon: schoolIcon }).bindPopup(`<b>${school?.name || "선택 학교"}</b><br/>학교 위치`);
        schoolMarker.addTo(map);
        layersRef.current.push(schoolMarker);
        ...
        if (origin?.lat && origin?.lon) {
          ...
          const originMarker = L.marker(originLatLng, { icon: originIcon }).bindPopup("출발지");
          originMarker.addTo(map);
          layersRef.current.push(originMarker);
        }
        ...
        if (selectedPin?.lat && selectedPin?.lon) {
          const pinLatLng = [Number(selectedPin.lat), Number(selectedPin.lon)];
          bounds.extend(pinLatLng);
          const pinIcon = L.divIcon({
            className: "safe-route-marker",
            html: markerIconHtml("R", "#e11d48", "📍"),
            iconSize: [34, 34],
            iconAnchor: [17, 34],
          });
          const pinMarker = L.marker(pinLatLng, { icon: pinIcon }).bindPopup("선택된 제보 위치");
          pinMarker.addTo(map);
        ...
        const fitItems = selectedPin?.lat && selectedPin?.lon ? bounds.pad(0.18) : bounds.pad(0.45);
        map.fitBounds(fitItems, { maxZoom: origin ? 16 : 17, animate: false });
        resizeTimer = window.setTimeout(() => map.invalidateSize(), 120);
        setLeafletStatus("지도를 확대·축소하거나 움직인 뒤 위험 위치를 탭하세요.");
      } catch (error) {
        console.warn("Report map failed:", error);
        setLeafletStatus("제보 지도를 불러오지 못했습니다. 네트워크 상태를 확인해 주세요.");
      }
    }
  ...
  }, [schoolLat, schoolLon, origin?.lat, origin?.lon, analysisRadius, selectedPin?.lat, selectedPin?.lon, setSelectedPin]);

  return <Card style={{ padding: 0, overflow: "hidden" }}><div style={{ padding: 18, borderBottom: "1px solid #e2e8f0" }}><h2 style={{ margin: 0 }}>제보 위치 선택</h2><p style={{ margin: "8px 0 0", color: "#64748b", lineHeight: 1.7, fontSize: 14 }}>지도를 움직이거나 확대·축소한 뒤 위험한 위치를 클릭하면 빨간 핀이 찍히고, 제보가 그 좌표와 함께 저장됩니다.</p></div><div className="report-location-map" style={{ position: "relative", height: 390, background: "#e2e8f0", touchAction: "pan-x pan-y", cursor: "crosshair" }}><div ref={mapEl} style={{ position: "absolute", inset: 0 }} />{!window.L ? <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", background: "rgba(248,250,252,0.72)", zIndex: 4, pointerEvents: "none" }}><div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 18, padding: "12px 16px", color: "#475569", fontWeight: 900 }}>{leafletStatus}</div></div> : null}<div style={{ position: "absolute", left: 16, bottom: 16, zIndex: 400, background: "rgba(255,255,255,0.94)", border: "1px solid #e2e8f0", borderRadius: 16, padding: "10px 12px", fontSize: 12, fontWeight: 900, color: selectedPin ? "#be123c" : "#475569", pointerEvents: "none" }}>{selectedPin ? `${selectedPin.lat.toFixed(6)}, ${selectedPin.lon.toFixed(6)}` : leafletStatus}</div></div><div style={{ padding: 14, display: "flex", gap: 8, flexWrap: "wrap" }}><button onClick={() => setSelectedPin({ lat: schoolLat, lon: schoolLon, label: "학교 주변" })} style={{ ...styles.button, background: "#e2e8f0", color: "#0f172a" }}>학교 주변으로 지정</button>{origin ? <button onClick={() => setSelectedPin({ lat: origin.lat, lon: origin.lon, label: "출발지 주변" })} style={{ ...styles.button, background: "#e2e8f0", color: "#0f172a" }}>출발지 주변으로 지정</button> : null}<button onClick={() => setSelectedPin(null)} style={{ ...styles.button, background: "#fff1f2", color: "#be123c" }}>핀 지우기</button></div></Card>;
}

function ReportForm({ onAddReport, selectedPin, setSelectedPin, school, origin, isSavingReport, sharedReportStatus }) {
...
function DevReportManager({ userReports, setUserReports, adminPassword }) {
  ...
  function clearReports() {
    setUserReports((prev) => (prev || []).filter((report) => report.shared));
    setManagerStatus("내 기기에만 저장된 제보를 전체 삭제했습니다. 공유 제보는 개별 삭제하세요.");
  }

  return <Card><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}><div><h2 style={{ margin: 0 }}>개발용 제보 관리</h2><p style={{ margin: "8px 0 0", color: "#64748b", lineHeight: 1.7 }}>테스트 중 등록한 위험 제보를 개별 삭제하거나 전체 초기화할 수 있습니다.</p></div><Badge tone={userReports?.length ? "red" : "gray"}>{userReports?.length || 0}건</Badge></div>{managerStatus ? <p style={{ margin: "10px 0 0", color: managerStatus.includes("실패") ? "#be123c" : "#166534", fontSize: 13, fontWeight: 900 }}>{managerStatus}</p> : null}{!userReports?.length ? <p style={{ color: "#64748b", lineHeight: 1.7 }}>삭제할 제보가 없습니다.</p> : <div style={{ display: "grid", gap: 10, marginTop: 14 }}>{userReports.map((report) => <div key={report.id} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}><div><b>{report.type || "위험 제보"} {report.shared ? <span style={{ color: "#0369a1", fontSize: 12 }}>· 공유</span> : <span style={{ color: "#94a3b8", fontSize: 12 }}>· 로컬</span>}</b><p style={{ margin: "6px 0 0", color: "#64748b", fontSize: 13 }}>{report.memo || "상세 설명 없음"}</p><p style={{ margin: "5px 0 0", color: "#94a3b8", fontSize: 12 }}>{report.locationLabel || "제보 위치"}{report.lat && report.lon ? ` · ${Number(report.lat).toFixed(5)}, ${Number(report.lon).toFixed(5)}` : ""}</p></div><button onClick={() => deleteReport(report.id)} style={{ ...styles.button, background: "#fee2e2", color: "#be123c", padding: "8px 10px", fontSize: 12 }}>삭제</button></div></div>)}</div>}{userReports?.length ? <button onClick={clearReports} style={{ ...styles.button, width: "100%", marginTop: 12, background: "#fff1f2", color: "#be123c" }}>내 기기 저장 제보 전체 삭제</button> : null}</Card>;
}
...
function BackendHealthCard() {
  ...
  const rows = health ? [
    ...
    { label: "Supabase 제보", ok: health.supabaseConfigured, detail: health.reportsTable ? `테이블 ${health.reportsTable}` : "공유 제보 DB" },
...
function App() {
  const { isAdmin, adminPassword, adminError, login: loginAdmin, logout: logoutAdmin } = useAdminAuth();
  ...
  const [sharedReportStatus, setSharedReportStatus] = useState("공유 제보 동기화 대기 중");
  ...
  useEffect(() => {
    ...
    async function loadSharedReports() {
      ...
      setSharedReportStatus("공유 제보 불러오는 중...");
      try {
        const params = new URLSearchParams();
        params.set("schoolId", selectedSchool.id);
        params.set("officeCode", selectedOffice?.code || selectedSchool.officeCode || "");
        params.set("limit", "120");
        params.set("sinceDays", "90");
        const response = await fetch(apiUrl(`/api/reports?${params.toString()}`), { cache: "no-store" });
        const data = await response.json().catch(() => ({}));
        if (!response.ok || data.ok === false) throw new Error(data.error || "공유 제보 조회 실패");
        const shared = Array.isArray(data.reports) ? data.reports.map(normalizeSharedReport) : [];
        if (cancelled) return;
        setUserReports((prev) => mergeReports((prev || []).filter((report) => !report.shared), shared));
        setSharedReportStatus(shared.length ? `공유 제보 ${shared.length.toLocaleString("ko-KR")}건 연결됨` : "이 학교의 공유 제보가 아직 없습니다.");
      } catch (error) {
        if (cancelled) return;
        setUserReports((prev) => (prev || []).filter((report) => !report.shared));
        setSharedReportStatus(`공유 제보 연결 실패: ${error?.message || "확인 필요"}`);
      }
    }
    loadSharedReports();
    return () => { cancelled = true; };
  }, [selectedSchool?.id, selectedOffice?.code]);
  ...
  async function handleAddReport(report) {
    ...
    setUserReports((prev) => [localReport, ...(prev || [])].slice(0, 120));
    setReportMascotFlash(true);
    window.setTimeout(() => setReportMascotFlash(false), 1800);
    setIsSavingReport(true);
    setSharedReportStatus("공유 제보 저장 중...");

    try {
      const response = await fetch(apiUrl("/api/reports"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(buildSharedReportPayload(localReport, selectedSchool, selectedOffice)),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.ok === false) throw new Error(data.error || "공유 제보 저장 실패");
      const saved = normalizeSharedReport(data.report || data);
      setUserReports((prev) => [saved, ...(prev || []).filter((item) => item.id !== optimisticId)].slice(0, 120));
      setSharedReportStatus("공유 제보가 다른 사용자에게도 저장되었습니다.");
    } catch (error) {
      setUserReports((prev) => (prev || []).map((item) => item.id === optimisticId ? { ...item, syncStatus: "내 기기에만 저장됨", shared: false } : item));
      setSharedReportStatus(`공유 저장 실패: ${error?.message || "내 기기에만 반영됨"}`);
    } finally {
  ...
  const tabs = [["today", "오늘의 등굣길"], ["guide", "활용 가이드"], ["routes", "경로 비교"], ["report", "위험 제보"], ["info", "정보"], ["settings", "설정"], ...(isAdmin ? [["admin", "관리"]] : [])];
