...
function DataSourceReferencesCard() {
  ...
  return <Card><h2 style={{ marginTop: 0 }}>사용 데이터 출처</h2><p style={{ color: "#64748b", lineHeight: 1.7, marginTop: -4 }}>서비스가 안전 점수와 권고문을 만들 때 사용하는 데이터 종류입니다.</p><div style={{ display: "grid", gap: 10 }}>{sources.map((source) => <div key={source.name} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 16, padding: 14 }}><b>{source.name}</b><p style={{ margin: "6px 0 0", color: "#64748b", fontSize: 13, lineHeight: 1.6 }}>{source.desc}</p></div>)}</div></Card>;
}
...
function App() {
  ...
  const baseScore = useMemo(() => {
    ...
    const routeSafetyScore = clamp(100 - norm(Math.max(0, routeKm - 1), 4) * 30, 0, 100);
    const accidentSafetyScore = clamp(100 - Number(publicSafetyAnalysis.accidentRisk || 0), 0, 100);
    const aiAuxPenalty = Math.min(Number(aiRisk?.score || 0) * 0.35, 10);
